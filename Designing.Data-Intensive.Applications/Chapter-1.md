...
规模较小的软件工程的代码可以是简洁且表达性强的，但是随着工程规模逐渐变大，代码经常会变得复杂且难以理解。这些复杂性会给所有需要在这个系统上工作的开发人员带来困扰，即增加了系统的维护成本。

复杂性的表现有很多：状态空间的爆炸；模块之间的紧耦合；纠缠不清的依赖关系；不一致的命名方式；为了解决性能问题而使用的hack代码；应对特殊场景问题的特殊逻辑等。

当复杂性使得系统维护困难时，预算和排期也会增加。在复杂软件中，修改时引入BUG的风险也会大大增加 。因此，降低复杂性可以显著地提高软件的可维护性，因此简单性应该作为构建系统的关键目标。

降低系统复杂性并不意味着要减少系统的功能，而是要减少不必要的复杂性。不必要的复杂性是指，并不是为了解决系统应该解决的问题而引入，仅仅是由于实现方式导致的复杂性。

减少不要复杂性的最好工具是**抽象**。良好的抽象可以在简洁、容易理解的表面下隐藏大量的实现细节。良好的抽象也可以用使用在大量不同的应用中。不仅可以提高代码复用的效率，还可以提升软件的质量。

然而，实现良好的抽象本身是困难的。在分布式系统领域，尽管有许多良好的算法，但是如何将这些算法封装为良好的抽象仍然并不十分清楚。

在本书中，我们将始终关注可以将庞大系统中的一部分提取为定义良好、可复用的组件的抽象。

### 演进能力

系统的要求不太可能是一成不变的，有多种可能的因素会导致系统的要求发生变化：出现了之前未预料到的场景；业务优先级变更；用户需要新的特性；新平台取代旧平台；法律监管的要求变更；系统演进迫使架构随之调整等。

系统的演进能力与系统的简洁性和抽象程度是紧密联系的，简单、容易理解的系统通常比复杂的系统更容易作出调整。

## 总结

在本章中，我们探讨了几种思考数据密集型系统的方法。这些准则将会引导我们进一步挖掘更深层次的技术细节。

应用程序需要满足各种各样的要求，包括**功能性要求**和**非功能性要求**。在本章中，我们详细讨论了非功能性要求中的**可靠性**、**可扩展性**和**可维护性**。

**可靠性**是指即使在错误发生时，系统仍然可以保持正常工作。错误可能是**硬件错误**、**软件错误**和**人为错误**。容错性机制可以对使用者隐藏特定类型的错误。

**可扩展性**是指即使在系统负载增加时，系统可以通过某种措施来保证良好的性能。为了描述可扩展性，我们首先对负载和性能做了定量描述。在可扩展的系统中，我们可以通过扩容的方式在高负载下保持可靠性。

**可维护性**包含多个方面，核心思想是让开发人员和维护团队工作地更加轻松。良好的抽象可以降低系统的复杂性，使系统在调整时更加灵活。良好的可操作性是指提供系统健康状态的可视化界面以及高效的管理工具。

构建可靠、可扩展、可维护的应用并不是一件容易的事情。在本书的后续章节中，将会通过一些例子来分析说明数据系统如何实现这些目标。
